...
//	private HashMap<Tuple<State, String>, State> stateMachine;
	...
	public StackDivide(AgentUtilsExtension utils, IAGOCoreVH agentCore, GameSpec game) {
		...
		stateMachine = new HashMap<StateEvent, State>();
		
		stateMachine.put(new StateEvent<State>(State.START, Event.EventClass.SEND_EXPRESSION, Event.SubClass.NONE), State.START);
		stateMachine.put(new StateEvent<State>(State.START, Event.EventClass.SEND_MESSAGE, null), State.END);
		stateMachine.put(new StateEvent<State>(State.END, Event.EventClass.SEND_MESSAGE, Event.SubClass.GENERIC_POS), State.START);
		this.stateEventController = new StateEventController<StackDivide.State>(stateMachine); // Have to create the controller after the hashmap
	...
	public LinkedList<Event> start(Event e){
		...
		for (StateEvent<State> eventType : eventTypes) {
			...
			if ((newState = stateMachine.get(eventType)) != null) {
