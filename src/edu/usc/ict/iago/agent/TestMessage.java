...
import edu.usc.ict.iago.agent.TestBehavior.LedgerBehavior;
...
public class TestMessage extends IAGOCoreMessage implements MessagePolicy {

	DateTimeFormatter dtf = DateTimeFormatter.ofPattern("HH:mm:ss");

	protected final String[] proposal = { "I think this deal is good for the both of us.",
			"I think you'll find this offer to be satisfactory.", "I think this arrangement is fair.",
			"I think this deal will interest you.", "Please consider this deal?" };

	protected final String[] acceptResponse = { "Great!", "Wonderful!", "I'm glad we could come to an agreement!",
			"Sounds good!" };

	protected final String[] rejectResponse = { "Oh that's too bad.", "Ah well, perhaps another time.",
			"Ok, maybe something different next time.", "Alright." };

	protected final String[] vhReject = { "I'm sorry, but I don't think that's fair to me.",
	...
	protected final String[] vhAccept = { "Your offer is good!", "That seems like a good deal.",
			"That will work for me.", "Yes. This deal will work." };

	protected final String[] vhWaiting = { "Hello? Are you still there?",
			"No rush, but are you going to send me an offer?",
			"Can I do anything to help us reach a deal that's good for us both?", "I'm sorry, but are you still there?",
			"Can I provide more information to help us reach consensus?" };

	public boolean isWithholding;
	public boolean lying;
	private TestBehavior.LedgerBehavior lb;
	private AgentUtilsExtension utils;
	public int opponentBATNA = -1;
	private int agentID;

	public void debug(String funcName) {
		System.out.println(dtf.format(LocalDateTime.now()) + " - " + "Message " + funcName);
	}
	...
	/***
	 * Constructor for a positive message. The resulting agent can be either
	 * withholding or open, lying or honest.
	 * 
	 * @param isWithholding a boolean representing whether an agent is withholding
	 *                      (true if yes, false if no)
	 * @param lying         a boolean representing whether an agent will tell a
	 *                      BATNA lie
	 * @param lb            an enum representing how the agent talks about favors
	 */
	public TestMessage(boolean isWithholding, boolean lying, TestBehavior.LedgerBehavior lb) {
		debug("TestMessage()");
	...
	private String getEmotionResponse(History history, GameSpec game, Event e) {
		debug("getEmotionResponse()");

		if (e.getType() != Event.EventClass.SEND_EXPRESSION)
			throw new UnsupportedOperationException(
					"The last event wasn't an expresion--this method is inappropriate.");
	...
	protected String getContradictionResponse(String drop) {
		debug("getContradictionResponse()");
	...
	public Event getVerboseMessageResponse(History history, GameSpec game, Event ePrime) {
		debug("getVerboseMessageResponse()");
		...
		if (p != null) // a preference was expressed
		{
			...
			if (p.isQuery() && this.isWithholding) { // asked about preferences
				...
				Event resp = new Event(agentID, Event.EventClass.SEND_MESSAGE, Event.SubClass.PREF_WITHHOLD, str,
						delay);
				return resp;
			}
		} else {
		...
		case GENERIC_POS:
			...
			if (best < 0) { // We do not have any guesses as to their favorite
				...
				str = "I agree!  Why don't we make sure you get your favorite item, and I get mine?  Yours is "
						+ game.getIssuePluralNames()[best] + ", right?";
				issue1 = best;
				relation = Relation.BEST;
				isQuery = true;
			}
		...
		case OFFER_REJECT:
			...
			if (offerCount > 0) {
				int avgPlayerValue = (Math.abs(
						utils.adversaryValueMax(lastOffer.getOffer()) - utils.adversaryValueMin(lastOffer.getOffer())))
						/ 2;
				if (Math.abs(utils.myActualOfferValue(lastOffer.getOffer()) - avgPlayerValue) > game.getNumIssues()
						* 2) {
					str = "Ok, I understand.  I do wish we could come up with something that is a more even split though.";
					if (best >= 0 && worst >= 0) {
						str += "  Isn't it true that you like " + game.getIssuePluralNames()[best] + " best and "
								+ game.getIssuePluralNames()[worst] + " least?";
			...
			break;
		case TIMING: // note: agent responds to this, but this event no longer is a user available
						// action
			sc = Event.SubClass.GENERIC_POS;
			int time = 0;
			int index = history.getHistory().size() - 1;
			...
			if (history.getHistory().size() > 1) {
				...
				str = "There is currently " + min + " minute" + (min == 1 ? "" : "s") + " and " + sec
						+ " seconds remaining.";
				...
				if (min > 0) {
					...
					str += "  AHH!  You're right!  Let's just split it like this: you get all the "
							+ game.getIssuePluralNames()[suggest2] + " and the " + game.getIssuePluralNames()[suggest3]
		...
		case THREAT_NEG: // "This is a warning, I'm about to walk away."
			...
			if (loweredBATNA != utils.myPresentedBATNA) {
				...
				str = "Well, maybe I can accept a little less. Would you be able to give me at least "
						+ utils.myPresentedBATNA + " points?";
				...
				str = "Oh well, I guess we really should walk away. Are you sure you can't accept anything less than "
						+ utils.adversaryBATNA + " points?";
				sc = Event.SubClass.BATNA_REQUEST;
			}
			break;
		case PREF_INFO:
		...
		case BATNA_INFO:
			if (ePrime.getValue() != -1) {
				...
				if (!utils.conflictBATNA(utils.myPresentedBATNA, utils.adversaryBATNA)) {
					...
					str += "In case you forgot, I already have an offer for " + utils.myPresentedBATNA
							+ " points, so anything that gets me more than " + utils.myPresentedBATNA
							+ " points will do.";
					value = utils.myPresentedBATNA;
					sc = Event.SubClass.BATNA_INFO;
				} else {
					opponentBATNA = utils.adversaryBATNA;
					str = "Well, since you can't accept anything less than " + utils.adversaryBATNA
							+ " points and I can't accept anything that gets me less than " + utils.myPresentedBATNA
							+ " points, I don't think we'll be able to make a deal. Maybe we should just walk away.";
					value = utils.myPresentedBATNA;
			...
			break;
		case BATNA_REQUEST:
			str += "I already have an offer for " + utils.myPresentedBATNA
					+ " points, so anything that gets me more than " + utils.myPresentedBATNA + " points will do.";
			value = utils.myPresentedBATNA;
			sc = Event.SubClass.BATNA_INFO;
			break;
		case CONFUSION:
